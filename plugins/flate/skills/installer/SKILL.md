...
2. `npm install @flate/sdk` i riktig workspace (eller pnpm/yarn hvis repoet bruker det).
3. Monter `<FlateBridge origin={process.env.NEXT_PUBLIC_FLATE_ORIGIN ?? "<portal-origin>"} />`:
   ...
   Import: `import { FlateBridge } from "@flate/sdk";`. SDK-en er no-op utenfor portalens iframe og på native.
